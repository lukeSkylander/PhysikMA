...
MAX_TRAIL_POINTS = 10000  # Maximum points to show in trajectory trails
...
Made with ❤️ for physics education and simulation"""
    ...
    def auto_screenshot():
        ...
        if not screenshot_taken and option_var.get() == "3D Pendulum":
            ...
            # Only export SVG projections for 3D pendulum
            params = dict(
                L=l_var.get(),
    ...
    def animate_pendulum3d(
        ...
        and coordinate axes. It includes theme-aware styling and performance
        optimizations to prevent flickering during longer animations.
        ...
        Args:
            ...
            show_trajectory: Boolean flag to display the pendulum's path
    ...
    def setup_pendulum():
        ...
        # FIXED: Always generate sine wave for comparison - but make it more accurate
        ...
        if initial_angle_deg < 30 and drag_val < 0.01:  # Small angle, low damping
            ...
            sine = np.cos(t) #phi[0] * np.cos(w0 * t)  # Analytical small-angle solution
        else:
            # For large angles or damped motion, use a theoretical reference
            # This shows what a simple harmonic oscillator would do
            w0 = np.sqrt(g_val / L_val)  # Theoretical frequency
            sine = np.cos(t)#np.radians(phi_var.get()) * np.cos(w0 * t)  # Simple harmonic reference
