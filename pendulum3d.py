...
def run(
    ...
    draw_mode : bool, optional
        When ``True`` the trajectory is drawn as a line; if ``False`` a scatter plot is used instead.
    force_arrows : bool, optional
        Draw arrows for gravity (blue), tension (red) and any applied impulse (green) at the bob's final position; all arrows are normalized to the same length so only their directions are depicted.
    """
