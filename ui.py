...
def main():
    ...
    # animation and plots. Allocate more width to the right column but ensure
    # the controls still have sufficient space on large screens.
    root.columnconfigure(0, weight=35)
    root.columnconfigure(1, weight=65)
    # Give both rows equal weight so top and bottom sections share vertical
    # space.
    ...
    def resize_ui(event):
        ...
        if run_button is not None:
            btn_width = min(MAX_BUTTON_WIDTH, max(1, ctrl_width // 30))
            run_button.configure(width=btn_width)
            if screenshot_button is not None:
                screenshot_button.configure(width=btn_width)
    ...
    slider_canvas = tk.Canvas(control_frame, highlightthickness=0, bg=dark_bg)
    slider_scrollbar = ttk.Scrollbar(control_frame, orient="vertical",
                                     command=slider_canvas.yview)
    slider_canvas.configure(yscrollcommand=slider_scrollbar.set)
    slider_canvas.grid(row=1, column=0, columnspan=3, padx=10, pady=10,
                       sticky="nsew")
    slider_scrollbar.grid(row=1, column=3, sticky="ns", pady=10)

    slider_frame = ttk.LabelFrame(slider_canvas, text="Pendulum Parameters")
    slider_window = slider_canvas.create_window((0, 0), window=slider_frame,
                                                anchor="nw")
    slider_frame.columnconfigure(1, weight=1)

    def _configure_slider(event):
        slider_canvas.configure(scrollregion=slider_canvas.bbox("all"))
    slider_frame.bind("<Configure>", _configure_slider)

    def _resize_slider(event):
        slider_canvas.itemconfigure(slider_window, width=event.width)
    slider_canvas.bind("<Configure>", _resize_slider)
    ...
    def update_inputs(*args):
        ...
        if pendulum_selected:
            ...
            slider_canvas.grid_remove()
            slider_scrollbar.grid_remove()
            state = tk.DISABLED
    ...
    def animate_two_lines(fig, ax, t, polar, azimuthal, speed, y_range=None):
        """Animate two line plots for polar and azimuthal angles."""
        line_theta, = ax.plot([], [], color="purple", label="θ")
        line_psi, = ax.plot([], [], color="red", label="ψ")
        ax.set_xlim(t[0], t[-1])
        if y_range is None:
            min_val = min(np.min(polar), np.min(azimuthal))
            max_val = max(np.max(polar), np.max(azimuthal))
        ...
        def init():
            line_theta.set_data([], [])
            line_psi.set_data([], [])
            return line_theta, line_psi

        def update(frame):
            line_theta.set_data(t[:frame], polar[:frame])
            line_psi.set_data(t[:frame], azimuthal[:frame])
            if frame == len(t) - 1:
                root.after(0, auto_screenshot)
            return line_theta, line_psi
    ...
    def animate_pendulum3d(
        ...
        corresponding velocities ``vx, vy, vz``. Forces (gravity, tension and an
        optional applied force) are computed using vector operations instead of
        angular quantities.
        """
    ...
    def run_selected():
        nonlocal ani, pendulum_data, ax_anim, screenshot_taken
        screenshot_taken = False
        selection = option_var.get()
        ax_plot.clear()
        ax_plot.set_facecolor(ax_bg)
        ...
        if selection == "RK4 Test 1":
            ...
            # Avoid division by zero when the pendulum passes through the poles
            theta_dot = np.divide(
                -vz,
                L_val * sin_theta,
                out=np.zeros_like(vz),
                where=sin_theta != 0,
            )
            phi_dot = np.divide(
                x * vy - y * vx,
            ...
            # Cartesian coordinates and velocities for animation
            ...
            if graph_var.get() == "Energy":
                ...
                ani.append(
                    animate_two_lines(
                        fig_plot, ax_plot, t, theta, psi, speed_var.get(),
                        (-3.5, 3.5)
                    )
                )
            else:
            ...
            if graph_var.get() == "Energy":
                ...
                ani.append(
                    animate_line(
                        fig_plot, ax_plot, t, dataset, speed_var.get(), y_range
                    )
                )
            ax_plot.set_xlabel('t')
    ...
    screenshot_button = ttk.Button(control_frame, text="Save Projections",
                                   command=take_screenshot)
    screenshot_button.grid(row=2, column=3, padx=10, pady=10, sticky="w")
    btn_width = min(MAX_BUTTON_WIDTH,
